...
class GAN:
    ...
    def __init__(self, options):
        ...
        self.g_bn1b = ops.batch_norm(name='g_bn1b')

        self.g_bn2 = ops.batch_norm(name='g_bn2')
        self.g_bn2b = ops.batch_norm(name='g_bn2b')
        ...
        self.d_bn2a = ops.batch_norm(name='d_bn2a')
        self.d_bn2 = ops.batch_norm(name='d_bn2')
        self.d_bn3a = ops.batch_norm(name='d_bn3a')
    ...
    def build_model(self):
        ...
        d_loss2 = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_wrong_image_logits, labels=tf.zeros_like(disc_wrong_image)))
        d_loss3 = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_fake_image_logits, labels=tf.zeros_like(disc_fake_image)))
    ...
    def discriminator(self, image, t_text_embedding):
        with tf.variable_scope("discriminator", reuse=tf.AUTO_REUSE):
            ...
            if self.options['vgg']:
                h0 = ops.lrelu(ops.conv2d(image, 3, stride = 1, name='d_h0a_conv'))
                h0 = ops.lrelu(ops.conv2d(h0, 3, stride = 2, name='d_h0_conv'))

                h1 = ops.lrelu(self.d_bn1a(ops.conv2d(h0, self.options['df_dim'] * 2, stride = 1, name='d_h1a_conv'))) 
                h1 = ops.lrelu(self.d_bn1(ops.conv2d(h0, self.options['df_dim'] * 2, stride = 2,  name='d_h1_conv')))                  

                h2 = ops.lrelu(self.d_bn2a(ops.conv2d(h1, self.options['df_dim'] * 4, stride = 1, name='d_h2a_conv'))) 
                h2 = ops.lrelu(self.d_bn2(ops.conv2d(h2, self.options['df_dim'] * 4, stride = 2, name='d_h2_conv')))                 

                h3 = ops.lrelu(self.d_bn3a(ops.conv2d(h2, self.options['df_dim'] * 8, stride = 1,  name='d_h3a_conv'))) 
                h3 = ops.lrelu(self.d_bn3(ops.conv2d(h3, self.options['df_dim'] * 8, stride = 2, name='d_h3_conv')))  

            else:
                if self.options['extra_64']:
                    image = ops.lrelu(ops.conv2d(image, 3, stride = 1, name='d_h0a_conv')) 
