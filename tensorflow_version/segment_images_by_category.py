...
def main ():
    ...
    parser.add_argument ('--orig', action='store_true', 
        help= 'keeps original image and does not process,\
               simply places in category folders')
    ...
    if CROP and ORIG:
        raise Exception ("Cannot perform both cropping and keep original image")
    ...
    if CROP:
        ...
        PROCESSED_DIR = os.path.join(MSCOCO_RAW_DIR, "processed_orig")
        DISREGARDED_DIR = os.path.join(MSCOCO_RAW_DIR, "disregarded_orig")
    else:
